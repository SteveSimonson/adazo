...
Primary transport is **Cloudflare Email Sending** (`hello@adazo.com`). GHL remains CRM; GHL mail is fallback only.

1. **Domain:** `npx wrangler email sending enable adazo.com` (done) + DNS for `cf-bounce` (see [cloudflare-email.md](./cloudflare-email.md)).
2. **From name:** `Adazo` / `hello@adazo.com` (wrangler `vars`).
3. **SPF + DKIM** on `cf-bounce.adazo.com`; **DMARC** on apex.
4. **List-Unsubscribe** header set in Worker send (`https://adazo.com/quiz`).
