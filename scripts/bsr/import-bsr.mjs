#!/usr/bin/env node
/**
 * Adazo Amazon BSR importer
 * ---------------------------
 * Pulls Top ~100 from Amazon Best Sellers leaf categories + beauty searches,
 * enriches ASINs, filters for beauty-relevant beauty products, and writes:
...
function isBeautyRelated(title = '', features = []) {
  const t = `${title} ${features.join(' ')}`.toLowerCase()
  if (!t.trim()) return false
  // Exclude obvious non-beauty noise from search results
  const exclude = /(men only|for men|boys? toy|video game|car wax|dog food)/i
...
function materialFamily(title = '', material = '') {
  ...
  if (/shampoo|conditioner|hair oil|dry shampoo|scalp|bond/.test(t)) return 'haircare'
  if (/serum|moisturizer|cleanser|sunscreen|spf|retinol|niacinamide|cream|toner|mask|lip/.test(t)) return 'skincare'
  return 'other'
}
...
function productFromListCard(card, meta, weekOf, expiresAt) {
  ...
  if (!isBeautyRelated(title, [])) return null
  ...
  return toProduct(
    {
      ...
      features: isBsr
        ...
        : [
            `Amazon beauty search · ${meta.bsrLabel || meta.adazoCategory}`,
            'Limited-time placement on Adazo this week',
            'Buy on Amazon — price and stock set by Amazon',
          ],
...
function toProduct(enriched, meta, weekOf, expiresAt) {
  ...
  const tagline =
    meta.source === 'bsr' && leafBsr
      ...
      : meta.source === 'bsr'
        ...
        : 'This week’s Amazon beauty picks · Limited-time options'
  ...
  return {
    ...
    description:
      meta.source === 'bsr'
        ? `${name} — selected from Amazon Best Sellers for Adazo’s weekly house edit. ${tagline}. Options rotate and are only available for a limited time; complete your purchase on Amazon.`
        : `${name} — selected from Amazon beauty search for Adazo’s weekly house edit. ${tagline}. Options rotate and are only available for a limited time; complete your purchase on Amazon.`,
    category: meta.adazoCategory,
      ...
      enriched.features?.length > 0
        ...
        : meta.source === 'bsr'
          ? [
              ...
              'Ships via Amazon',
              'Limited-time placement on Adazo',
            ]
          : [
              'Found via Amazon beauty search',
              'Ships via Amazon',
              'Limited-time placement on Adazo',
            ],
    specs,
    ...
    ...(enriched.reviewCount != null
      ...
      : {}),
    hue: (hueBase[meta.adazoCategory] || 80) + (meta.rank % 40),
...
async function main() {
  ...
  console.log(`\nAdazo BSR import · weekOf=${weekOf}`)
  ...
  const candidates = [] // { asin, rank, source, bsrLabel, bsrId, adazoCategory, collection }
  ...
  for (const cat of config.categories.filter((c) => c.enabled)) {
    ...
    for (const r of ranks) {
      candidates.push({
        ...
        bsrLabel: cat.label,
        bsrId: cat.id,
        adazoCategory: cat.adazoCategory,
        collection: cat.collection,
        beautyBias: cat.beautyBias,
      })
    }
  }
  ...
  const listBamboo = unique.filter(
    (c) => c.title && isBeautyRelated(c.title, []),
  )
  ...
  unique.sort((a, b) => {
    ...
    const aHas = a.title && isBeautyRelated(a.title, []) ? 0 : 1
    const bHas = b.title && isBeautyRelated(b.title, []) ? 0 : 1
    return (
      aHas - bHas ||
      (bias[a.beautyBias] ?? 2) - (bias[b.beautyBias] ?? 2) ||
      a.rank - b.rank
    )
  })
  ...
  for (let i = 0; i < toEnrich.length; i++) {
    ...
    if (!enriched?.title) {
      if (meta.title && isBeautyRelated(meta.title, [])) {
    ...
    const materialBeauty = /bamboo/i.test(enriched.material || '')
    const keep =
      isBeautyRelated(enriched.title, enriched.features) ||
      materialBeauty ||
      (meta.source === 'search' && isBeautyRelated(enriched.title, []))

    if (!keep) {
      console.log(`no-beauty: ${enriched.title.slice(0, 50)}`)
      continue
    }
  ...
  try {
    ...
    if (existsSync(rawPath)) {
      ...
      for (const a of prior) {
        ...
        if (!a.title || !isBeautyRelated(a.title, a.features || [])) continue
        const meta = {
          asin: a.asin,
          rank: 50 + seeded,
          source: 'search',
          ...
          bsrId: 'prior-scrape',
          adazoCategory: a.category || 'kitchen',
  ...
  // 4) Fill each Adazo category to quota (default 20) via extra beauty searches
  const quota = Number(
  ...
  console.log(`\n== Quota fill (target ${quota} beauty items per category)`)
  ...
  for (const cat of allCats) {
    ...
    for (const q of queries) {
      ...
      for (const r of ranks) {
        ...
        if (!r.title || !isBeautyRelated(r.title, [])) continue
        const meta = {
          asin: r.asin,
          rank: 100 + n,
          title: r.title,
