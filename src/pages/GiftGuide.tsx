...
export function GiftGuidePage() {
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        <Seo
          ...
          description="This gift guide is not on Adazo."
          path={`/gifts/${slug || ''}`}
          noindex
        />
        <h1 className="font-display text-3xl font-semibold">Not found</h1>
        <Link to="/gifts" className="text-bamboo font-semibold mt-4 inline-block">
          Back to gifts
        </Link>
      </div>
    )
  ...
  const heroSrc =
    guide.heroImage ||
    (entries[0]
      ? primaryImage(entries[0].product) || entries[0].product.images?.[0]
      : undefined)
    ...
    <div className="pb-24">
      ...
      {heroSrc ? (
        <div className="relative h-[min(40vh,26rem)] overflow-hidden bg-paper-2">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-paper via-paper/40 to-charcoal/20" />
        </div>
      ) : null}

      <article
        className={`mx-auto max-w-3xl px-4 sm:px-6 py-10 ${heroSrc ? 'relative -mt-14' : ''}`}
      >
        <Link
          to="/gifts"
