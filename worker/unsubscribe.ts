...
async function validToken(token: string, secret: string): Promise<string | null> {
  ...
  return verified ? contactId : null
}
...
export function addUnsubscribeFooter(parts: EmailParts, unsubscribeUrl: string): EmailParts {
  return {
    ...parts,
    html: `${parts.html}<hr style="border:0;border-top:1px solid #e5e7eb;margin:32px 0 16px"><p style="font:12px/1.5 Arial,sans-serif;color:#6b7280">You received this because you completed the Adazo Vibe Check. <a href="${unsubscribeUrl}">Unsubscribe from future emails</a>.</p>`,
    text: `${parts.text}\n\n---\nYou received this because you completed the Adazo Vibe Check. Unsubscribe: ${unsubscribeUrl}`,
  }
}
...
export async function handleUnsubscribe(
  ...
  if (request.method !== 'POST') {
    return jsonResponse(site, request, { ok: false, error: 'Method not allowed' }, 405)
  }
  ...
  const result = await fetch(
    `https://services.leadconnectorhq.com/contacts/${encodeURIComponent(contactId)}`,
    {
      ...
      body: JSON.stringify({
        dnd: true,
        dndSettings: {
          Email: { status: 'active', message: 'Unsubscribed', code: '' },
        },
      }),
    },
  )
